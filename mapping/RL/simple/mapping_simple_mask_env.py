...
class MappingSimpleMaskEnv(gym.Env):
    def __init__(self, render_mode='human', use_image=False):
        super(MappingSimpleMaskEnv, self).__init__()
    ...
    def step(self, action):
        ...
        cur_compute, direction, width, height  = action
        cur_compute += 1
        width += 1
        height += 1
        self.current_step += 1
        ...
        is_valid, col_start, row_start, col_end, row_end = self._is_valid_rectangle(
                        direction, base_col, base_row, col_start, row_start, col_end, row_end)
    ...
    def _is_valid_rectangle(self, direction, base_col, base_row, col_start, row_start, col_end, row_end):
        ...
        for rect in self.rect_list:
            valid, new_left, new_top, new_right, new_bottom = self.bypass(direction, base_col, base_row, rect_new, rect)
            if not valid:
                return False, new_left, new_top, new_right, new_bottom
            else:
                rect_new = [new_left, new_top, new_right, new_bottom]
    ...
    def bypass(self, direction, base_col, base_row, rect_new, rect):
        ...
        if direction == 0:
            ...
            top_bypass = top_new
            right_bypass = right_new
            bottom_bypass = max(bottom_new, top-1)
        elif direction == 3:
            assert base_col == right_new, base_row == top_new
            left_bypass = max(left_new, left+1)
            top_bypass = top_new
            right_bypass = right_new
            bottom_bypass = bottom_new
